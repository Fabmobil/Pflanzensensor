...
#ifdef ESP32
#include <SoftwareSerial.h> // EspSoftwareSerial library on ESP32
#else
#include <SoftwareSerial.h> // ESP8266 SoftwareSerial
#endif
...
// Timing constants
static constexpr unsigned long SDS011_COMMAND_TIMEOUT = 1000; // 1 second timeout
...
struct SDS011Config : public SensorConfig {
   ...
   */
  SDS011Config() : rxPin(SDS011_RX_PIN), txPin(SDS011_TX_PIN), warmupTime(SDS011_WARMUP_TIME) {
    name = F("SDS011");
...
public:
   ...
   */
  explicit SDS011Sensor(const SDS011Config& config, class SensorManager* sensorManager);
  /**
   * @brief Destructor
   * @override
   */
...
private:
  ...
  // Helper methods
  const SDS011Config& sds011Config() const { return static_cast<const SDS011Config&>(config()); }
