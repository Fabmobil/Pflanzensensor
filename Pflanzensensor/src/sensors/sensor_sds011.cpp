...
SDS011Sensor::SDS011Sensor(const SDS011Config& config, SensorManager* sensorManager)
  ...
  ThresholdDefaults pm10Defaults = {0, 0, SDS011_PM10_GREEN_HIGH, SDS011_PM10_YELLOW_HIGH};
  ...
  ThresholdDefaults pm25Defaults = {0, 0, SDS011_PM25_GREEN_HIGH, SDS011_PM25_YELLOW_HIGH};
...
SensorResult SDS011Sensor::init() {
  ...
  logDebug(F("Testing sensor communication..."));
  if (!testCommunication()) {
    logDebug(F("Communication test failed - sensor may not be connected or pins may "
               "be wrong"));
    // Don't fail initialization yet, just log the warning
  } else {
    logDebug(F("Communication test successful"));
  }
...
SDS011Status SDS011Sensor::readResponse(unsigned long timeout) {
  ...
  logDebug(String(F("Starting to read response, timeout: ")) + String(timeout) + F("ms"));

  while ((millis() - startTime) < timeout && bytesRead < SDS011_RESPONSE_LENGTH) {
    if (m_serial->available()) {
      uint8_t b = m_serial->read();
      m_response[bytesRead] = b;

      logDebug(String(F("Read byte ")) + String(bytesRead) + String(F(": 0x")) + String(b, HEX));

      switch (bytesRead) {
      case 0:
        if (b != SDS011_HEAD) {
          logDebug(String(F("Invalid head byte: expected 0x")) + String(SDS011_HEAD, HEX) +
                   String(F(" got 0x")) + String(b, HEX));
          return SDS011Status::InvalidHead;
        }
        break;
      case 1:
        if (b != SDS011_RESPONSE_ID && b != SDS011_REPORT_ID) {
          logDebug(String(F("Invalid response ID: expected 0x")) + String(SDS011_RESPONSE_ID, HEX) +
                   String(F(" or 0x")) + String(SDS011_REPORT_ID, HEX) + String(F(" got 0x")) +
                   String(b, HEX));
      ...
      case 8:
        if (b != checksum) {
          logDebug(String(F("Invalid checksum: calculated 0x")) + String(checksum, HEX) +
                   String(F(" got 0x")) + String(b, HEX));
      ...
      case 9:
        if (b != SDS011_TAIL) {
          logDebug(String(F("Invalid tail byte: expected 0x")) + String(SDS011_TAIL, HEX) +
                   String(F(" got 0x")) + String(b, HEX));
  ...
  if (bytesRead < SDS011_RESPONSE_LENGTH) {
    logDebug(String(F("Timeout reading response: got ")) + String(bytesRead) +
             String(F(" bytes, expected ")) + String(SDS011_RESPONSE_LENGTH));
    return SDS011Status::NotAvailable;
  }

  logDebug(F("Successfully read complete response"));
...
bool SDS011Sensor::fetchSample(float& value, size_t index) {
  logDebug(String(F("Fetching sample for index ")) + String(index));
  if (!isInitialized()) {
    logDebug(F(": Attempted to fetch sample without initialization"));
    value = NAN;
    return false;
  }
  ...
  if (!sendCommand(SDS011_QUERY_CMD, 0x00, 0x00)) {
    logDebug(F(": Failed to send query command"));
    value = NAN;
    return false;
  }
  ...
  if (status != SDS011Status::Ok) {
    logDebug(String(F(": Failed to read response, status: ")) + String((int)status));
    value = NAN;
    return false;
  }
  ...
  logDebug(String(F("Fetched value: ")) + String(value));
  return !isnan(value);
}
...
bool SDS011Sensor::wakeup() {
  logDebug(F("Starting fan for warmup/measurement"));

  // First, check if the sensor is already awake by trying to read data
  logDebug(F("Checking if sensor is already awake..."));
  ...
  while ((millis() - startCheck) < 100) {
    if (m_serial->available()) {
      uint8_t b = m_serial->read();
      logDebug(String(F("Found existing data: 0x")) + String(b, HEX));
      hasData = true;
    }
    delay(1);
  }
  ...
  if (hasData) {
    logDebug(F("Sensor appears to already be awake"));
    m_state.sleeping = false;
    return true;
  }

  // Log the command being sent
  logDebug(String(F("Sending wakeup command: SLEEP_CMD=")) + String(SDS011_SLEEP_CMD, HEX) +
           String(F(" WORK_MODE=")) + String(SDS011_WORK_MODE, HEX));

  if (!sendCommand(SDS011_SLEEP_CMD, SDS011_WORK_MODE, 0x00)) {
    logDebug(F("Failed to send wakeup command"));
    return false;
  }
  ...
  for (size_t i = 0; i < SDS011_COMMAND_LENGTH; i++) {
    cmdStr += String(m_command[i], HEX) + String(F(" "));
  }
  logDebug(cmdStr);

  // Read and validate response
  SDS011Status status = readResponse();
  logDebug(String(F("Wakeup response status: ")) + String((int)status));

  if (status != SDS011Status::Ok) {
    logDebug(String(F("Fan start command failed with status: ")) + String((int)status));
    return false;
  }

  m_state.sleeping = false;
  logDebug(F("Sensor successfully woken up"));
  return true;
}

bool SDS011Sensor::sleep() {
  logDebug(F("Stopping fan to preserve sensor life"));
  if (!sendCommand(SDS011_SLEEP_CMD, SDS011_SLEEP_MODE, 0x00)) {
    logDebug(F("Failed to send sleep command"));
    return false;
  }
  ...
  SDS011Status status = readResponse();
  if (status != SDS011Status::Ok) {
    logDebug(String(F("Fan stop command failed with status: ")) + String((int)status));
    return false;
  }
...
uint8_t SDS011Sensor::calculateChecksum(const uint8_t* data, size_t length) const {
  uint8_t sum = 0;
...
SensorResult SDS011Sensor::startMeasurement() { return SensorResult::success(); }

SensorResult SDS011Sensor::continueMeasurement() { return SensorResult::success(); }
...
bool SDS011Sensor::testCommunication() {
  ...
  if (m_serial->available()) {
    ...
    logDebug(String(F("Received echo: 0x")) + String(echo, HEX));
  } else {
  ...
  logDebug(String(F("Test communication response status: ")) + String((int)status));
