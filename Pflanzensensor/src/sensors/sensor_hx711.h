...
public:
   ...
   */
  explicit HX711Sensor(const HX711Config& config, class SensorManager* sensorManager);
   ...
   * @override
   */
  bool isValidValue(float value) const override { return !isnan(value) && value >= 0.0f; }
  ...
  SharedHardwareInfo getSharedHardwareInfo() const override {
    return SharedHardwareInfo(SensorType::HX711, hx711Config().doutPin, hx711Config().minimumDelay);
  }
...
private:
  static constexpr size_t REQUIRED_SAMPLES = 3; ///< Number of samples required for a valid reading

  // Hardware configuration
  const HX711Config& hx711Config() const { return static_cast<const HX711Config&>(config()); }
