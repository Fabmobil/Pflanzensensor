...
struct MHZ19Config : public SensorConfig {
  ...
  unsigned long warmupTime; ///< Warmup time in milliseconds before valid readings
  float range;              ///< Detection range in ppm (2000, 5000, or 10000)
  ...
  MHZ19Config() {
    ...
    range = MHZ19_RANGE;
#else
    range = 2000.0f; // Default to 2000ppm range for backwards compatibility
#endif
  }
};
...
public:
   ...
   */
  explicit MHZ19Sensor(const MHZ19Config& config, class SensorManager* sensorManager);
  ...
  bool isValidValue(float value) const override {
    // Validity check should only reject impossible values.
    // Threshold handling (green/yellow/red) is configured separately.
  ...
  bool requiresWarmup(unsigned long& warmupTime) const override {
    warmupTime = m_mhz19Config.warmupTime; // Already in milliseconds
    return true;
  }
  ...
  SharedHardwareInfo getSharedHardwareInfo() const override {
    return SharedHardwareInfo(SensorType::MHZ19, m_mhz19Config.pwmPin, m_mhz19Config.minimumDelay);
  }
  ...
  /**
   * @brief Handle sensor errors without hard-deinitializing MH-Z19 on transient
   * PWM glitches
   */
  void handleSensorError() override;
