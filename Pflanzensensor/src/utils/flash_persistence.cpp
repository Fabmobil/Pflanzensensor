...
uint32_t FlashPersistence::getSafeOffset() {
  ...
  uint32_t safeOffset = ((sketchSize + FLASH_SECTOR_SIZE - 1) / FLASH_SECTOR_SIZE + SAFETY_MARGIN_SECTORS) * FLASH_SECTOR_SIZE;
  
  // Ensure we're still within the sketch partition
  uint32_t sketchEnd = ESP.getFreeSketchSpace() + sketchSize;
  if (safeOffset + MAX_CONFIG_SIZE > sketchEnd) {
    logger.error(F("FlashPers"), F("Insufficient flash space for config storage"));
    return 0;
  }
  
  logger.debug(F("FlashPers"), F("Safe offset: ") + String(safeOffset) + 
               F(" (sketch: ") + String(sketchSize) + F(" bytes)"));
  
  return safeOffset;
}

uint32_t FlashPersistence::getMaxSize() {
...
ResourceResult FlashPersistence::saveToFlash(const String& jsonData) {
  ...
  if (dataSize > MAX_CONFIG_SIZE - 16) { // Reserve 16 bytes for header
    return ResourceResult::fail(ResourceError::INSUFFICIENT_SPACE,
  ...
  uint32_t sectorsNeeded = ((dataSize + 16 + FLASH_SECTOR_SIZE - 1) / FLASH_SECTOR_SIZE);
  logger.debug(F("FlashPers"), F("Erasing ") + String(sectorsNeeded) + F(" sectors..."));
  
  for (uint32_t i = 0; i < sectorsNeeded; i++) {
    uint32_t sectorAddr = (offset + i * FLASH_SECTOR_SIZE) / FLASH_SECTOR_SIZE;
...
ResourceResult FlashPersistence::loadFromFlash(String& jsonData) {
  ...
  if (magic != MAGIC_NUMBER) {
    ...
    return ResourceResult::fail(ResourceError::NOT_FOUND,
                               F("No valid config in flash"));
  }
  ...
  if (version != VERSION) {
    ...
    return ResourceResult::fail(ResourceError::VERSION_MISMATCH,
                               F("Incompatible config version"));
  }
  ...
  if (dataSize == 0 || dataSize > MAX_CONFIG_SIZE) {
    return ResourceResult::fail(ResourceError::VALIDATION_ERROR,
  ...
  if (!buffer) {
    return ResourceResult::fail(ResourceError::OUT_OF_MEMORY,
...
ResourceResult FlashPersistence::clearFlash() {
  ...
  uint32_t sectorAddr = offset / FLASH_SECTOR_SIZE;
  if (!ESP.flashEraseSector(sectorAddr)) {
    return ResourceResult::fail(ResourceError::OPERATION_FAILED,
