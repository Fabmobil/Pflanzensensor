...
void StartpageHandler::renderWiFiSetupForm() {
  ...
  if (WiFi.getMode() == WIFI_AP || WiFi.getMode() == WIFI_AP_STA) {
    ...
    sendChunk(F("<select name='wifi_slot' id='wifi_slot' required "
                "class='form-control'>"));
    ...
    sendChunk(F("<select name='wifi_ssid' id='wifi_ssid' required "
                "class='form-control'>"));
    ...
    if (networkCount > 0) {
      for (int i = 0; i < networkCount && i < 20; ++i) { // Limit to 20 networks
        ...
        // Format security
        String security = (encType == ENC_TYPE_NONE) ? "Offen" : "Verschlüsselt";

        sendChunk(F("<option value='") + networkSSID + F("'>"));
        sendChunk(networkSSID + F(" (") + signalStrength + F(", ") + security + F(")"));
        sendChunk(F("</option>"));
      }
    } else {
    ...
    sendChunk(F("<input type='password' name='wifi_password' id='wifi_password' "
                "required placeholder='WiFi-Passwort'>"));
    sendChunk(F("</div>"));

    // Submit button
    sendChunk(F("<button type='submit' class='button button-primary'>WiFi "
