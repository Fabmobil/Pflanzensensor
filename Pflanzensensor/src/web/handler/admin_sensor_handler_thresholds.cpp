...
bool AdminSensorHandler::processThresholds(Sensor* sensor,
  ...
  if (updateThreshold(id + "_" + String(measurementIdx), "yellowLow",
                      currentLimits.yellowLow, newValue)) {
    newLimits.yellowLow = newValue;
    updated = true;
  }
  if (updateThreshold(id + "_" + String(measurementIdx), "greenLow",
                      currentLimits.greenLow, newValue)) {
    newLimits.greenLow = newValue;
    updated = true;
  }
  if (updateThreshold(id + "_" + String(measurementIdx), "greenHigh",
                      currentLimits.greenHigh, newValue)) {
    newLimits.greenHigh = newValue;
    updated = true;
  }
  if (updateThreshold(id + "_" + String(measurementIdx), "yellowHigh",
                      currentLimits.yellowHigh, newValue)) {
  ...
  logger.info(F("AdminSensorHandler"),
        F("Schwellenwerte aktualisiert f\u00fcr ") + id + F("[") +
          String(measurementIdx) + F("]: ") +
          String(newLimits.yellowLow, 2) + F(", ") +
          String(newLimits.greenLow, 2) + F(", ") +
          String(newLimits.greenHigh, 2) + F(", ") +
          String(newLimits.yellowHigh, 2));
  }
  return updated;
}

bool AdminSensorHandler::updateThreshold(const String& baseId,
                                         const String& thresholdName,
...
void AdminSensorHandler::handleThresholds() {
  ...
  if (!sensor->isInitialized()) {
    logger.error(F("AdminSensorHandler"),
                 F("Sensor nicht initialisiert: ") + sensorId);
    sendJsonResponse(
  ...
  int n = sscanf(thresholdsCsv.c_str(), "%f,%f,%f,%f", &thresholds[0],
                 &thresholds[1], &thresholds[2], &thresholds[3]);

  // Debug: print parsed threshold values
  logger.debug(F("AdminSensorHandler"), F("Parsed thresholds: n=") + String(n) +
                                            F(", values=") +
                                            String(thresholds[0], 2) + F(",") +
                                            String(thresholds[1], 2) + F(",") +
                                            String(thresholds[2], 2) + F(",") +
                                            String(thresholds[3], 2));

  if (n != 4) {
    logger.error(F("AdminSensorHandler"),
                 F("Ungültiges Schwellenwert-Format: ") + thresholdsCsv);
    sendJsonResponse(
        400, F("{\"success\":false,\"error\":\"Ungültiges Schwellenwert-Format\"}"));
    return;
  }
  ...
  logger.debug(F("AdminSensorHandler"),
               F("Limits before: ") + String(limits.yellowLow, 2) + F(",") +
  ...
  logger.debug(F("AdminSensorHandler"),
               F("Limits after: ") + String(limits.yellowLow, 2) + F(",") +
                   String(limits.greenLow, 2) + F(",") +
                   String(limits.greenHigh, 2) + F(",") +
                   String(limits.yellowHigh, 2));
  ...
  if (changed) {
    ...
    auto& limits2 =
        sensor->mutableConfig().measurements[measurementIndex].limits;
    ...
    auto result = SensorPersistence::updateSensorThresholds(
        sensorId, measurementIndex, limits.yellowLow, limits.greenLow,
    ...
    if (!result.isSuccess()) {
      ...
      sendJsonResponse(
          500,
          F("{\"success\":false,\"error\":\"Fehler beim Speichern der Schwellenwerte\"}"));
      return;
    }
    ...
    logger.info(
        ...
        F("Thresholds updated for sensor ") + sensorId + F(" measurement ") +
            String(measurementIndex) + F(": ") + String(thresholds[0], 2) +
            F(",") + String(thresholds[1], 2) + F(",") +
            String(thresholds[2], 2) + F(",") + String(thresholds[3], 2));
  }
