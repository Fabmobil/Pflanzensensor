...
WebRouter::WebRouter(ESP8266WebServer& server) : _server(server) {
  if (!hasEnoughMemory()) {
    logger.error(F("WebRouter"),
                 F("Nicht genügend Speicher für WebRouter-Initialisierung"));
    return;
  }
...
RouterResult WebRouter::addRoute(HTTPMethod method, const String& url,
                                 HandlerCallback handler) {
  if (!hasEnoughMemory()) {
  logger.error(F("WebRouter"), F("Nicht genügend Speicher für Route: ") + url);
  return RouterResult::fail(RouterError::RESOURCE_ERROR,
                F("Nicht genügend Speicher"));
  }

  if (url.isEmpty() || !handler) {
  logger.error(F("WebRouter"), F("Ungültige Routen-Parameter für: ") + url);
  return RouterResult::fail(RouterError::INVALID_ROUTE,
                F("Ungültige Routen-Parameter"));
  }
  ...
  return RouterResult::fail(RouterError::INVALID_METHOD,
          F("Nicht unterstützte Methode"));
    }
...
void WebRouter::serveStatic(const String& urlPrefix, fs::FS& fs,
                            const String& path, bool cache) {
  ...
  _server.serveStatic(urlPrefix.c_str(), fs, path.c_str(),
                      cache ? "max-age=3600" : nullptr);
...
bool WebRouter::executeMiddleware(HTTPMethod method, const String& url) {
  for (const auto& mw : _middleware) {
    logger.debug(F("WebRouter"), String(F("Führe Middleware aus für: ")) +
...
Route* WebRouter::findRoute(HTTPMethod method, const String& url) {
  logger.debug(F("WebRouter"), F("Suche nach Route: ") +
                                   methodToString(method) + F(" ") + url);
  logger.debug(F("WebRouter"),
               String(F("Insgesamt registrierte Routen: ")) + String(_routes.size()));

  for (auto& route : _routes) {
  logger.debug(F("WebRouter"), F("Prüfe Route: ") +
                   methodToString(route.method) + F(" ") +
                   route.url);
    if (route.url == url && route.method == method) {
  logger.debug(F("WebRouter"), F("Gefundene passende Route: ") +
               methodToString(method) + F(" ") + url);
      return &route;
    }
  }
  logger.debug(F("WebRouter"), F("Keine passende Route gefunden für: ") +
                                   methodToString(method) + F(" ") + url);
  return nullptr;
}

void WebRouter::logRouteRegistration(HTTPMethod method, const String& url) {
  logger.debug(F("WebRouter"), String(F("Route erfolgreich registriert: ")) +
                                   methodToString(method) + " " + url);
}
