...
void WebseiteDebugAusgeben() {
  ...
  String html = "<h2>Logs</h2>";
  html += "<div class=\"tuerkis\">";

  html += "<form action='/SetzeLogLevel' method='post'>";
  html += "<select name='logLevel'>";
  html += "<option value='DEBUG'" + String(logger.getLogLevel() == LogLevel::DEBUG ? " selected" : "") + ">DEBUG</option>";
  html += "<option value='INFO'" + String(logger.getLogLevel() == LogLevel::INFO ? " selected" : "") + ">INFO</option>";
  html += "<option value='WARNING'" + String(logger.getLogLevel() == LogLevel::WARNING ? " selected" : "") + ">WARNING</option>";
  html += "<option value='ERROR'" + String(logger.getLogLevel() == LogLevel::ERROR ? " selected" : "") + ">ERROR</option>";
  html += "</select>";
  html += "<input type='submit' value='Setze Log Level'>";
  html += "</form>";

  html += "<div id='logTable'>";
  html += logger.getLogsAsHtmlTable(logAnzahlWebseite);
  html += "</div>";
  html += "</div>";
  ...
  setInterval(updateLogTable, 5000); // Update logs every 5 seconds
  // Initial updates
  updateLogs();
