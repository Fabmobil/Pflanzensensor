/**
 * @file sensors.js
 * @brief JavaScript for pixel art plant sensor interface
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initializeSensorUpdates();
    setupMenuNavigation();
});

function initializeScrollAnimation() {
    // Auto-scroll from cloud to flower after 3 seconds
    setTimeout(() => {
    ...
    if (cloudSection) {
        cloudSection.addEventListener('click', scrollToFlower);
        cloudSection.style.pointerEvents = 'auto';
        cloudSection.style.cursor = 'pointer';
    }
}

function scrollToFlower() {
    scrollAnimationActive = true;
    const flowerSection = document.querySelector('.flower-section');
    if (flowerSection) {
        flowerSection.scrollIntoView({
            behavior: 'smooth',
            block: 'start'
        });
    }
...
function initializeSensorUpdates() {
...
function updateSensorValues() {
  ...
  return fetch('/getLatestValues')
    ...
    .then(data => {
      ...
      // Update sensor values in pixel art design
      if (data.sensors) {
        latestSensorData = data.sensors; // Store for timer updates
        Object.entries(data.sensors).forEach(([fieldName, sensorData]) => {
          updatePixelSensorDisplay(fieldName, sensorData);
        });
      }

      // Update system info in menu section
      if (data.systemInfo) {
        updateSystemInfo(data.systemInfo);
      }

      console.log('Sensor values updated successfully');
      return data;
    })
    .catch(error => {
      updateFailureCount++;
      ...
      if (updateFailureCount >= MAX_UPDATE_FAILURES) {
        console.error('Max update failures reached, showing error state');
        showConnectionError();
      }
    });
}
